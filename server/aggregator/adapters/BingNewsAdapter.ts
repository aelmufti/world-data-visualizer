/**
 * Bing News RSS Adapter
 * 
 * Fetches articles from Bing News RSS feed
...
/**
 * Adapter for Bing News RSS feed
 * 
 ...
 */
export class BingNewsAdapter implements NewsSourceAdapter {
  readonly name = 'Bing News'
  private baseUrl = 'https://www.bing.com/news/search'
   ...
   * Bing News RSS is public, so always configured
   */
  ...
  /**
   * Fetch articles from Bing News RSS
   * 
   ...
   * 
   * Requirements:
   * - 1.2: Support Bing News RSS as a news source
  ...
  async fetchArticles(keywords: string[], maxResults: number = 10): Promise<Article[]> {
    ...
    try {
      ...
      url.searchParams.append('format', 'rss')
      ...
      if (!response.ok) {
        ...
        const errorMessage = `Bing News HTTP error: ${response.status} ${response.statusText}${errorText ? ` - ${errorText}` : ''}`
      ...
      if (!err.message.startsWith('Bing News')) {
  ...
  private normalizeArticle(
    ...
    // Generate unique ID: bing-{hash of title}
