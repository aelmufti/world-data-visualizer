/**
 * Unit tests for BingNewsAdapter
 * 
 * Tests RSS parsing, article normalization, and error handling
 */

import { describe, it, expect, beforeEach, vi } from 'vitest'
import { BingNewsAdapter } from './BingNewsAdapter'

describe('BingNewsAdapter', () => {
  let adapter: BingNewsAdapter

  beforeEach(() => {
    adapter = new BingNewsAdapter()
  ...
  describe('Configuration', () => {
    ...
    it('should have correct name', () => {
      expect(adapter.name).toBe('Bing News')
    })
  })
...
<rss version="2.0">
  <channel>
    ...
    <item>
      ...
      expect(articles[0].source).toBe('Bing News')
      expect(articles[0].url).toBe('https://example.com/article1')
      expect(articles[0].id).toMatch(/^bing-[a-f0-9]{12}$/)
      ...
      expect(articles[1].source).toBe('Bing News')
    })
...
<rss version="2.0">
  <channel>
    ...
    it('should handle HTTP errors', async () => {
      ...
      await expect(adapter.fetchArticles(['test'], 10)).rejects.toThrow('Bing News HTTP error: 500')
...
<rss version="2.0">
  <channel>
    ...
    <item>
      ...
      expect(articles[0].id).toMatch(/^bing-/)
      expect(articles[1].id).toMatch(/^bing-/)
    })
  })
})
